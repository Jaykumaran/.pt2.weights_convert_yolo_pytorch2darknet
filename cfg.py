...
def parse_cfg(cfgfile):
    def erase_comment(line):
        ...
        return line
    blocks = []
    cfgfile = '/content/drive/MyDrive/EDU VID Sample/yolov8.cfg'
    fp = open(cfgfile, 'r')
    block =  None
    line = fp.readline()
    while line != '':
...
def save_conv(fp, conv_model):
    header = torch.IntTensor([0, 1, 0, 0])
    header.numpy().tofile(fp)

    weights = conv_model.weight.data.numpy()
    weights = weights.reshape(-1)
    weights.tofile(fp)

    if conv_model.bias is not None:
        biases = conv_model.bias.data.numpy()
